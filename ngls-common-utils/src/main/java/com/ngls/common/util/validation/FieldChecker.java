...
package com.ngls.common.util.validation;

import com.ngls.common.util.basic.SimpleStringUtils;
import com.ngls.common.util.exception.IllegalParamException;
import com.ngls.common.util.exception.RequiredParamException;
...
public class FieldChecker {
    ...
    public static void checkMinMaxLengthNotCheckEmpty(String value, int minLength,
                                         int maxLength, String fieldName){
        if (StringUtils.isNotBlank(value)&& (value.length() < minLength || value.length() > maxLength)) {
            throw new IllegalParamException(
                    "'" + fieldName + "'" + "的长度必须大于等于" + minLength + "，且小于等于" + maxLength);
        }
    }
    ...
    public static void checkOnlyNumber(String value,String fieldName){
        checkEmpty(value,fieldName);
        if(SimpleStringUtils.isNumber(value)){
            throw new IllegalParamException("'" + fieldName + "'不能为纯数字");
        }
    }
